...
NEW_CODES: list[tuple[str, int, str, str, str]] = [
    ...
    ("cloudflared.invalid_token", 400, "invalid Cloudflare tunnel token; paste the connector token from Zero Trust → Tunnels (it starts with eyJ)", "Cloudflare 隧道 Token 无效；请从 Zero Trust → Tunnels 粘贴以 eyJ 开头的连接器令牌", "Cloudflare トンネルトークンが無効です。Zero Trust → Tunnels から eyJ で始まるコネクタートークンを貼り付けてください"),
    ("cloudflared.start_failed", 503, "the tunnel process died after start: {error}", "隧道进程启动后立即退出：{error}", "トンネルプロセスは起動直後に終了しました: {error}"),
