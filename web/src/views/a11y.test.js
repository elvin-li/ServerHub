...
describe('backup and workload surface leftovers', () => {
  ...
  it('announces the Apps managed and autostart count breakdowns', () => {
    ...
    const apps = readFileSync(resolve(SRC, 'views/Apps.vue'), 'utf8')
    expect(apps).toMatch(/class="meta-count" role="status" v-if="asRecord\(managed\)\.counts"/)
...
describe('llm, photos, vpn and health surface leftovers', () => {
  ...
  it('spells the WireGuard ping outcome, not just its LED colour', () => {
    ...
    expect(wireguard).toMatch(
      /class="sr-only">\{\{ asRecord\(r\)\.reachable \? t\('wg\.reachable'\) : t\('wg\.unreachable'\) \}\}/,
    )
  })
...
describe('operations polling and submission guards', () => {
  ...
  it('discards leftover timer, watcher and copy-to-clipboard writes after leave', () => {
    ...
    expect(ollama).toContain('v-if="asRecord(m).content || asRecord(m).pending"')
...
describe('leftover Infinity interpolations', () => {
  ...
  it('VncConsole session limits go through finiteSecs', () => {
    ...
    expect(vnc).toMatch(/expires:\s*finiteSecs\(asRecord\(sessionInfo\)\.expires_in\)/)
    expect(vnc).toMatch(/max:\s*finiteSecs\(asRecord\(sessionInfo\)\.max_session_seconds\)/)
    expect(vnc).toMatch(/finiteText\(asRecord\(vm\)\.name\)/)
    expect(vnc).not.toMatch(/vm\.console\?\.protocol \|\| 'VNC'/)
    expect(vnc).toMatch(/finiteText\(asRecord\(asRecord\(vm\)\.console\)\.protocol/)
    ...
    expect(vnc).toMatch(/finiteText\(asRecord\(event\.detail\)\.reason, ''\) \|\| t\('vms\.console_connection_failed'\)/)
  ...
  it('LineChart drops leftover Infinity samples instead of labelling them', () => {
    ...
    expect(chart).toMatch(/finiteText\(asRecord\(g\)\.label\)/)
    expect(chart).not.toMatch(/\{\{\s*s\.name\s*\}\}/)
    expect(chart).toMatch(/finiteText\(asRecord\(s\)\.name\)/)
  ...
  it('Containers leftover engine figures go through finiteN', () => {
    ...
    expect(containers).toMatch(/finiteText\(asRecord\(data\)\.update_checked_at/)
    ...
    expect(containers).toMatch(/finiteText\(asRecord\(im\)\.Repository/)
    expect(containers).not.toMatch(/inspectData\.State\?\.Health \|\| '—'/)
    expect(containers).toMatch(/finiteText\(asRecord\(asRecord\(inspectData\)\.State\)\.Health/)
    ...
    expect(containers).toMatch(/finiteText\(asRecord\(c\)\.project\)/)
    expect(containers).not.toMatch(/\{\{\s*v\.Driver\s*\}\}/)
    expect(containers).toMatch(/finiteText\(asRecord\(v\)\.Driver\)/)
    expect(containers).not.toMatch(/\{\{\s*v\.Mountpoint\s*\}\}/)
    expect(containers).toMatch(/finiteText\(asRecord\(v\)\.Mountpoint\)/)
    expect(containers).not.toMatch(/\{\{\s*n\.Scope\s*\}\}/)
    expect(containers).toMatch(/finiteText\(asRecord\(n\)\.Scope\)/)
    ...
    expect(containers).toMatch(/logName\.value = finiteText\(asRecord\(c\)\.name/)
  ...
  it('Apps leftover catalog counts go through finiteN', () => {
    ...
    expect(apps).toMatch(/finiteN\(asRecord\(asRecord\(managed\)\.counts\)\.total\)/)
    ...
    expect(apps).toMatch(/finiteText\(asRecord\(remoteInfo\)\.last_check/)
    expect(apps).not.toMatch(/\{\{\s*remoteInfo\.last_check\s*\}\}/)
    expect(apps).toMatch(/finiteText\(asRecord\(c\)\.ports\)/)
    expect(apps).not.toMatch(/\{\{\s*c\.ports\s*\}\}/)
    expect(apps).toMatch(/finiteText\(asRecord\(it\)\.ports_summary/)
    expect(apps).not.toMatch(/\{\{\s*it\.detail \|\| it\.plist \|\| '—'\s*\}\}/)
    expect(apps).not.toMatch(/finiteText\(it\.detail \|\| it\.plist\)/)
    expect(apps).toMatch(/finiteText\(asRecord\(it\)\.detail, ''\) \|\| finiteText\(asRecord\(it\)\.plist/)
    expect(apps).not.toMatch(/o\.version \|\| '—'/)
    expect(apps).toMatch(/finiteText\(asRecord\(o\)\.version/)
    expect(apps).toMatch(/finiteText\(asRecord\(detail\)\.uuid/)
    expect(apps).not.toMatch(/\{\{\s*tpl\.name\s*\}\}/)
    expect(apps).toMatch(/finiteText\(asRecord\(tpl\)\.name\)/)
    expect(apps).not.toMatch(/\{\{\s*it\.name\s*\}\}/)
    expect(apps).toMatch(/finiteText\(asRecord\(it\)\.name\)/)
    expect(apps).not.toMatch(/\{\{\s*detail\.name\s*\}\}/)
    expect(apps).toMatch(/finiteText\(asRecord\(detail\)\.name\)/)
    expect(apps).not.toMatch(/n\.ip \|\| '—'/)
    expect(apps).toMatch(/finiteText\(asRecord\(n\)\.ip\)/)
    expect(apps).not.toMatch(/tpl\.desc \|\| '—'/)
    expect(apps).toMatch(/finiteText\(asRecord\(tpl\)\.desc\)/)
    expect(apps).not.toMatch(/it\.path \|\| it\.package \|\| it\.backend \|\| '—'/)
    expect(apps).toMatch(/finiteText\(asRecord\(it\)\.path, ''\) \|\| finiteText\(asRecord\(it\)\.package, ''\) \|\| finiteText\(asRecord\(it\)\.backend\)/)
    ...
    expect(apps).toMatch(/asArray\(asRecord\(it\)\.ips\)\.map\(n => finiteText\(n, ''\)\)/)
    expect(apps).not.toMatch(/\{\{\s*cfMsg\s*\}\}/)
    expect(apps).toMatch(/finiteText\(cfMsg\)/)
    expect(apps).not.toMatch(/\(detail\.env_sample \|\| \[\]\)\.join\('\\n'\)/)
    expect(apps).toMatch(/asArray\(asRecord\(detail\)\.env_sample\)\.map\(n => finiteText\(n, ''\)\)/)
    ...
    expect(apps).toMatch(/finiteText\(asRecord\(managed\.value\)\.host_ip, ''\)/)
    ...
    expect(apps).toMatch(/finiteText\(asRecord\(installTpl\)\.method, ''\) \|\| 'system'/)
    expect(apps).not.toMatch(/installTpl\.package \? ` · \$\{installTpl\.package\}`/)
    expect(apps).toMatch(/finiteText\(asRecord\(installTpl\)\.package, ''\) \? ` · \$\{finiteText\(asRecord\(installTpl\)\.package\)\}`/)
    expect(apps).not.toMatch(/it\.policy \? ' · ' \+ it\.policy/)
    expect(apps).toMatch(/finiteText\(asRecord\(it\)\.policy, ''\) \? ' · ' \+ finiteText\(asRecord\(it\)\.policy\)/)
    expect(apps).not.toMatch(/\{\{\s*it\.kind\s*\}\}/)
    expect(apps).toMatch(/finiteText\(asRecord\(it\)\.kind\)/)
    expect(apps).not.toMatch(/\{\{\s*installTpl\.notes\s*\}\}/)
    expect(apps).toMatch(/finiteText\(asRecord\(installTpl\)\.notes\)/)
    expect(apps).not.toMatch(/\{\{\s*v\.help\s*\}\}/)
    expect(apps).toMatch(/finiteText\(asRecord\(v\)\.help\)/)
    expect(apps).not.toMatch(/tpl\.remote_version \? ` \$\{tpl\.remote_version\}`/)
    expect(apps).toMatch(/finiteText\(asRecord\(tpl\)\.remote_version, ''\) \? ` \$\{finiteText\(asRecord\(tpl\)\.remote_version\)\}`/)
  ...
  it('Ollama leftover context lengths, pids and dates reject Infinity', () => {
    ...
    expect(ollama).toMatch(/finiteN\(asRecord\(m\)\.context_length\)/)
    expect(ollama).toMatch(/finiteN\(asRecord\(asRecord\(data\)\.service\)\.pid\)/)
    expect(ollama).toMatch(/function fmtDate\([\s\S]*Number\.isFinite/)
    expect(ollama).toMatch(/finiteText\(asRecord\(m\)\.parameter_size/)
    expect(ollama).not.toMatch(/m\.parameter_size \? ' ' \+ m\.parameter_size/)
    expect(ollama).not.toMatch(/data\.version \|\| '—'/)
    expect(ollama).toMatch(/finiteText\(asRecord\(data\)\.version/)
    expect(ollama).not.toMatch(/\{\{\s*m\.name\s*\}\}/)
    expect(ollama).toMatch(/finiteText\(asRecord\(m\)\.name\)/)
    expect(ollama).not.toMatch(/data\.service\?\.label \|\| '—'/)
    expect(ollama).toMatch(/finiteText\(asRecord\(asRecord\(data\)\.service\)\.label/)
    expect(ollama).not.toMatch(/m\.family \|\| '—'/)
    expect(ollama).toMatch(/finiteText\(asRecord\(m\)\.family/)
    ...
    expect(ollama).toMatch(/asArray\(asRecord\(m\)\.capabilities\)\.map\(c => finiteText\(c, ''\)\)/)
    expect(ollama).not.toMatch(/class="badge cap">\{\{ c \}\}<\/span>/)
    expect(ollama).toMatch(/finiteText\(c\)/)
    expect(ollama).toMatch(/v-for="c in asArray\(asRecord\(m\)\.capabilities\)"/)
  ...
  it('MainArray leftover capacities go through fmtGb/withUnit', () => {
    ...
    expect(array).toMatch(/finiteText\(asRecord\(asRecord\(m\)\.smart\)\.temp\)/)
    expect(array).toMatch(/finiteText\(asRecord\(d\)\.size\)/)
    expect(array).toMatch(/finiteText\(asRecord\(m\)\.size\)/)
    ...
    expect(array).toMatch(/finiteN\(asRecord\(a\)\.id\)/)
    expect(array).toMatch(/finiteText\(asRecord\(a\)\.value\)/)
    expect(array).toMatch(/finiteText\(asRecord\(a\)\.worst\)/)
    expect(array).toMatch(/finiteText\(asRecord\(a\)\.thresh\)/)
    expect(array).toMatch(/finiteText\(asRecord\(a\)\.raw\)/)
    ...
    expect(array).toMatch(/finiteText\(asRecord\(v\)\.id\)/)
    expect(array).not.toMatch(/\{\{\s*m\.id\s*\}\}/)
    expect(array).toMatch(/finiteText\(asRecord\(m\)\.id\)/)
    expect(array).not.toMatch(/v\.disk_id \|\| '—'/)
    expect(array).not.toMatch(/v\.disk_id \? ' · ' \+ v\.disk_id/)
    expect(array).toMatch(/finiteText\(asRecord\(v\)\.disk_id/)
    expect(array).toMatch(/finiteText\(asRecord\(d\)\.disk_id/)
    expect(array).not.toMatch(/d\.protocol \|\| '—'/)
    expect(array).toMatch(/finiteText\(asRecord\(d\)\.protocol\)/)
    ...
    expect(array).toMatch(/finiteText\(asRecord\(d\)\.label\)/)
    expect(array).not.toMatch(/m\.smart\?\.model \|\| m\.smart\?\.serial \|\| '—'/)
    expect(array).toMatch(/finiteText\(asRecord\(asRecord\(m\)\.smart\)\.model, ''\) \|\| finiteText\(asRecord\(asRecord\(m\)\.smart\)\.serial\)/)
    expect(array).not.toMatch(/v\.fs \|\| '—'/)
    expect(array).toMatch(/finiteText\(asRecord\(v\)\.fs\)/)
    ...
    expect(array).toMatch(/asArray\(asRecord\(asRecord\(m\)\.caps\)\.supported\)\.map\(n => finiteText\(n, ''\)\)/)
    ...
    expect(array).toMatch(/finiteText\(asRecord\(asRecord\(m\)\.caps\)\.reason\)/)
    expect(array).not.toMatch(/\{\{\s*m\.lastResult\s*\}\}/)
    expect(array).toMatch(/finiteText\(asRecord\(m\)\.lastResult\)/)
  ...
  it('Pool leftover capacities go through fmtGb/withUnit', () => {
    ...
    expect(pool).toMatch(/fmtGb\(asRecord\(m\)\.used_gb\)/)
    expect(pool).toMatch(/withUnit\(asRecord\(m\)\.pct, '%'\)/)
    expect(pool).not.toMatch(/\{\{\s*r\.at_risk_gb\s*\}\} GB/)
    expect(pool).toMatch(/fmtGb\(asRecord\(r\)\.at_risk_gb\)/)
    expect(pool).toMatch(/fmtGb\(asRecord\(r\)\.survives_gb\)/)
    expect(pool).not.toMatch(/m\.disk_id \|\| '—'/)
    expect(pool).not.toMatch(/c\.disk_id \|\| '—'/)
    expect(pool).not.toMatch(/r\.disk_id \|\| '—'/)
    expect(pool).toMatch(/finiteText\(asRecord\(m\)\.disk_id/)
    expect(pool).toMatch(/finiteText\(asRecord\(c\)\.disk_id/)
    expect(pool).toMatch(/finiteText\(asRecord\(r\)\.disk_id/)
    expect(pool).not.toMatch(/m\.filesystem \|\| '—'/)
    expect(pool).not.toMatch(/c\.filesystem \|\| '—'/)
    expect(pool).not.toMatch(/shownTarget \|\| '—'/)
    expect(pool).toMatch(/finiteText\(asRecord\(m\)\.filesystem\)/)
    expect(pool).toMatch(/finiteText\(asRecord\(c\)\.filesystem\)/)
  ...
  it('WireGuard leftover ports/mtu/latency go through finite helpers', () => {
    ...
    expect(wg).toMatch(/withUnit\(asRecord\(r\)\.latency_ms, ' ms'\)/)
    ...
    expect(wg).toMatch(/finiteText\(asRecord\(asRecord\(data\)\.wstunnel\)\.listen\)/)
    expect(wg).toMatch(/finiteText\(asRecord\(asRecord\(data\)\.wstunnel\)\.client_command\)/)
    expect(wg).toMatch(/asRecord\(asRecord\(data\)\.wstunnel\)\.running \? t\('common\.running'\)/)
  ...
  it('VMs leftover ids go through finiteText', () => {
    ...
    expect(vms).toMatch(/finiteText\(asRecord\(renameTarget\)\.id\)/)
    expect(vms).not.toMatch(/\{\{\s*v\.name\s*\}\}/)
    expect(vms).toMatch(/finiteText\(asRecord\(v\)\.name\)/)
    expect(vms).not.toMatch(/\{\{\s*cloneTarget\.name\s*\}\}/)
    expect(vms).toMatch(/finiteText\(asRecord\(cloneTarget\)\.name\)/)
    ...
    expect(vms).toMatch(/finiteText\(asRecord\(data\.value\)\.host_ip/)
    expect(vms).toMatch(/finiteText\(asRecord\(row\)\.name/)
    expect(vms).toMatch(/finiteText\(asRecord\(out\)\.message/)
    expect(vms).toMatch(/finiteText\(window\.location\.hostname, ''\) \|\| finiteText\(asRecord\(data\.value\)\.host_ip, ''\)/)
  ...
  it('Modules leftover names go through finiteText', () => {
    ...
    expect(modules).toMatch(/finiteText\(asRecord\(m\)\.name\)/)
    expect(modules).toMatch(/finiteText\(asRecord\(m\)\.description\)/)
  ...
  it('Health leftover summary counts go through finiteN', () => {
    ...
    expect(health).toMatch(/finiteN\(asRecord\(asRecord\(data\)\.summary\)\.ok\)/)
    expect(health).toMatch(/finiteN\(asRecord\(asRecord\(data\)\.summary\)\.warn\)/)
    expect(health).toMatch(/finiteN\(asRecord\(asRecord\(data\)\.summary\)\.error\)/)
    expect(health).toMatch(/finiteN\(asRecord\(asRecord\(data\)\.summary\)\.total\)/)
    expect(health).toMatch(/finiteText\(data\?\.ts/)
    expect(health).toMatch(/finiteText\(errText\(asRecord\(c\)\.detail\)\)/)
    expect(health).not.toMatch(/\{\{\s*errText\(c\.detail\)\s*\}\}/)
    expect(health).not.toMatch(/\{\{\s*c\.name\s*\}\}/)
    expect(health).toMatch(/finiteText\(asRecord\(c\)\.name\)/)
  ...
  it('Maintenance leftover rc and finished reject Infinity', () => {
    ...
    expect(maintenance).toMatch(/finiteN\(asRecord\(task\)\.rc\)/)
    expect(maintenance).toMatch(/finiteText\(asRecord\(task\)\.finished\)/)
    expect(maintenance).toMatch(/finiteN\(j\.rc\)/)
    expect(maintenance).not.toMatch(/\{\{\s*task\.id\s*\}\}/)
    expect(maintenance).toMatch(/finiteText\(asRecord\(task\)\.id\)/)
    expect(maintenance).not.toMatch(/\{\{\s*task\.name\s*\}\}/)
    expect(maintenance).toMatch(/finiteText\(asRecord\(task\)\.name\)/)
  ...
  it('Account leftover recovery counts go through finiteN', () => {
    ...
    expect(account).toMatch(/name: finiteText\(asRecord\(authState\)\.username\)/)
    expect(account).not.toMatch(/\{\{\s*authState\.username\s*\}\}/)
    expect(account).toMatch(/finiteText\(asRecord\(authState\)\.username\)/)
  ...
  it('Bookmarks leftover summary counts go through finite helpers', () => {
    ...
    expect(bookmarks).toMatch(/finiteN\(asRecord\(data\)\.up/)
    expect(bookmarks).toMatch(/finiteN\(asRecord\(data\)\.down/)
    expect(bookmarks).toMatch(/finiteText\(asRecord\(data\)\.checked_at/)
    expect(bookmarks).not.toMatch(/at: data\.checked_at \|\| '—'/)
    expect(bookmarks).not.toMatch(/\{\{\s*b\.name\s*\}\}/)
    expect(bookmarks).toMatch(/finiteText\(asRecord\(b\)\.name\)/)
    expect(bookmarks).toMatch(/finiteText\(asRecord\(b\)\.url\)/)
    expect(bookmarks).not.toMatch(/return b\.status \|\| t\('dashboard\.bm_up'\)/)
    expect(bookmarks).toMatch(/finiteText\(asRecord\(b\)\.status, ''\) \|\| t\('dashboard\.bm_up'\)/)
    expect(bookmarks).toMatch(/finiteText\(bk\.name, ''\) \|\| finiteText\(bk\.id, ''\)/)
    expect(bookmarks).not.toMatch(/:href="b\.url"/)
    expect(bookmarks).toMatch(/:href="finiteText\(asRecord\(b\)\.url, ''\)"/)
  ...
  it('Alerts leftover timestamps go through fmtTs', () => {
    ...
    expect(alerts).toMatch(/finiteText\(asRecord\(a\)\.name\)/)
  ...
  it('Backups leftover ports/files/mtime go through finite helpers', () => {
    ...
    expect(backups).toMatch(/finiteN\(asRecord\(asRecord\(layers\)\.db\)\.port/)
    expect(backups).toMatch(/finiteN\(asRecord\(asRecord\(layers\)\.bridge\)\.exported_files/)
    expect(backups).toMatch(/finiteN\(asRecord\(asRecord\(job\)\.params\)\.retain/)
    expect(backups).toMatch(/finiteText\(rsyncBinary\.version/)
    expect(backups).toMatch(/finiteText\(asRecord\(asRecord\(preview\)\.binary\)\.version/)
    expect(backups).toMatch(/finiteText\(asRecord\(asRecord\(job\)\.params\)\.stack_id/)
    expect(backups).not.toMatch(/\{\{\s*job\.name\s*\}\}/)
    expect(backups).toMatch(/finiteText\(asRecord\(job\)\.name\)/)
    expect(backups).not.toMatch(/\{\{\s*b\.name\s*\}\}/)
    expect(backups).toMatch(/finiteText\(asRecord\(b\)\.name\)/)
    expect(backups).not.toMatch(/layers\.db\?\.last\?\.name \|\| t\('photoshub\.never'\)/)
    expect(backups).toMatch(/finiteText\(asRecord\(asRecord\(asRecord\(layers\)\.db\)\.last\)\.name/)
    expect(backups).not.toMatch(/layers\.bridge\?\.path \|\| '—'/)
    expect(backups).toMatch(/finiteText\(asRecord\(asRecord\(layers\)\.bridge\)\.last_success, ''\) \|\| finiteText\(asRecord\(asRecord\(layers\)\.bridge\)\.path\)/)
    ...
    expect(backups).toMatch(/asArray\(postgresTargets\.value\)\.map\(\(t\) => finiteText\(asRecord\(t\)\.id, ''\)\)/)
    expect(backups).not.toMatch(/\{\{\s*job\.cron\s*\}\}/)
    expect(backups).toMatch(/finiteText\(asRecord\(job\)\.cron\)/)
  ...
  it('Audit leftover extra fields go through finiteText', () => {
    ...
    expect(audit).toMatch(/finiteText\(asRecord\(e\)\.username\)/)
    expect(audit).not.toMatch(/e\.client \|\| '—'/)
    expect(audit).toMatch(/finiteText\(asRecord\(e\)\.client\)/)
    expect(audit).not.toMatch(/e\.outcome \|\| '—'/)
    expect(audit).toMatch(/finiteText\(asRecord\(e\)\.outcome\)/)
  ...
  it('ScheduleJobForm leftover preview counts go through finiteN', () => {
    ...
    expect(form).toMatch(/finiteN\(asRecord\(preview\)\.creates\)/)
    expect(form).toMatch(/finiteN\(asRecord\(preview\)\.updates\)/)
    expect(form).toMatch(/finiteN\(asRecord\(preview\)\.deletes\)/)
    ...
    expect(form).toMatch(/v-for="\(line, i\) in asArray\(asRecord\(preview\)\.samples\)"/)
  })
  ...
  it('ServiceDetailDrawer leftover detail goes through finiteText', () => {
    ...
    expect(drawer).toMatch(/finiteText\(asRecord\(service\)\.detail\)/)
    expect(drawer).not.toMatch(/\{\{\s*service\.id\s*\}\}/)
    expect(drawer).toMatch(/finiteText\(asRecord\(service\)\.id\)/)
    expect(drawer).not.toMatch(/service\.group \|\| '—'/)
    expect(drawer).toMatch(/finiteText\(asRecord\(service\)\.group\)/)
    expect(drawer).not.toMatch(/service\.url \|\| '—'/)
    expect(drawer).toMatch(/finiteText\(asRecord\(service\)\.url\)/)
    expect(drawer).not.toMatch(/\{\{\s*service\.name\s*\}\}/)
    expect(drawer).toMatch(/finiteText\(asRecord\(service\)\.name\)/)
    expect(drawer).not.toMatch(/\(service\.env_sample \|\| \[\]\)\.join\('\\n'\)/)
    expect(drawer).toMatch(/asArray\(asRecord\(service\)\.env_sample\)\.map\(n => finiteText\(n, ''\)\)/)
    expect(drawer).not.toMatch(/\{\{\s*service\.launchctl\s*\}\}/)
    expect(drawer).toMatch(/finiteText\(asRecord\(service\)\.launchctl\)/)
    ...
    expect(drawer).toMatch(/asArray\(ad\.ports\)\.map\(\(n\) => finiteText\(n, ''\)\)/)
    expect(drawer).toMatch(/asArray\(sc\.ports\)\.map\(\(n\) => finiteText\(n, ''\)\)/)
  })
  ...
  it('NotifyChannels leftover channel ids go through finiteText', () => {
    ...
    expect(ch).toMatch(/finiteText\(asRecord\(c\)\.id\)/)
    expect(ch).not.toMatch(/editing\.name \|\| editing\.id/)
    expect(ch).toMatch(/finiteText\(asRecord\(editing\)\.id/)
    expect(ch).not.toMatch(/\{\{\s*c\.name\s*\}\}/)
    expect(ch).toMatch(/finiteText\(asRecord\(c\)\.name\)/)
  ...
  it('AssistantDrawer leftover path/title interpolations go through finiteText', () => {
    ...
    expect(ad).toMatch(/finiteText\(asRecord\(p\)\.path\)/)
    expect(ad).not.toMatch(/\{\{\s*p\.title\s*\}\}/)
    expect(ad).toMatch(/finiteText\(asRecord\(p\)\.title\)/)
  ...
  it('ServiceLogsModal leftover source goes through finiteText', () => {
    ...
    expect(modal).toMatch(/finiteText\(asRecord\(entry\)\.source\)/)
  ...
  it('ServiceSignatures leftover ports and counts go through finite helpers', () => {
    ...
    expect(sigs).toMatch(/finiteText\(asRecord\(row\)\.slug\)/)
    expect(sigs).not.toMatch(/\{\{\s*row\.category\s*\}\}/)
    expect(sigs).toMatch(/finiteText\(asRecord\(row\)\.category\)/)
  ...
  it('GroupRules leftover ids and matcher lists go through finite helpers', () => {
    ...
    expect(grules).toMatch(/finiteText\(asRecord\(row\)\.group\)/)
    expect(grules).not.toMatch(/\{\{\s*row\.id\s*\}\}/)
    expect(grules).toMatch(/finiteText\(asRecord\(row\)\.id\)/)
  ...
  it('Compose leftover stack names go through finiteText', () => {
    ...
    expect(compose).toMatch(/finiteText\(asRecord\(s\)\.name\)/)
    expect(compose).not.toMatch(/\{\{\s*s\.status\s*\}\}/)
    expect(compose).toMatch(/finiteText\(asRecord\(s\)\.status\)/)
    expect(compose).not.toMatch(/s\.path \|\| '—'/)
    expect(compose).toMatch(/finiteText\(asRecord\(s\)\.path\)/)
  ...
  it('Terminal leftover container labels and session ids go through finiteText', () => {
    ...
    expect(terminal).toMatch(/finiteText\(asRecord\(item\)\.label, ''\) \|\| finiteText\(container\.value, ''\)/)
    ...
    expect(terminal).toMatch(/finiteText\(asRecord\(c\)\.label/)
  })
