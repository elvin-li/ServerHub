...
describe('backup and workload surface leftovers', () => {
  ...
  it('announces the Apps managed and autostart count breakdowns', () => {
    ...
    expect(apps).toMatch(/class="meta-count" role="status" v-if="recGet\(autostart, 'counts'\)"/)
...
describe('workload operation semantics', () => {
  ...
  it('shows partial container batch failures as failures', () => {
    ...
    expect(containers).toContain("recGet(j, 'ok') === false")
  })
...
describe('operations polling and submission guards', () => {
  ...
  it('keeps file navigation and terminal connection state current', () => {
    ...
    expect(files.match(/if \(!recGet\(j, 'ok'\)\) throw new Error/g)).toHaveLength(2)
...
describe('leftover Infinity interpolations', () => {
  ...
  it('Containers leftover engine figures go through finiteN', () => {
    ...
    expect(containers).toMatch(/`❌ \$\{finiteText\(recGet\(j, 'message'\)\)\}`/)
  ...
  it('Apps leftover catalog counts go through finiteN', () => {
    ...
    expect(apps).toMatch(/finiteN\(recGet\(recGet\(autostart, 'counts'\), 'autostart_on'\)/)
    expect(apps).not.toMatch(/\{\{\s*managed\.counts\.total\s*\}\}/)
    expect(apps).toMatch(/finiteN\(recGet\(overview.value, 'total'\), null\)/)
    expect(apps).toMatch(/finiteN\(recGet\(overview.value, 'native_count'\), null\)/)
    expect(apps).toMatch(/finiteN\(recGet\(overview.value, 'docker_count'\), null\)/)
  ...
  it('MainArray leftover capacities go through fmtGb/withUnit', () => {
    ...
    expect(array).toMatch(/id: finiteText\(recGet\(row, 'id'\)\)/)
    expect(array).not.toMatch(/\$\{t\('main_extra\.mount'\)\} \$\{v\.id\}/)
    expect(array).toMatch(/\$\{t\('main_extra\.mount'\)\} \$\{finiteText\(recGet\(row, 'id'\)\)\}/)
    expect(array).not.toMatch(/v\.mount \|\| t\('main_extra\.not_mounted'\)/)
    expect(array).toMatch(/finiteText\(recGet\(row, 'mount'\), ''\) \|\| t\('main_extra\.not_mounted'\)/)
    ...
    expect(array).toMatch(/reasons: finiteText\(recGet\(d, 'error'\)\)/)
  ...
  it('WireGuard leftover ports/mtu/latency go through finite helpers', () => {
    ...
    expect(wg).toMatch(/finiteN\(recGet\(data, 'mtu'\)\)/)
    expect(wg).toMatch(/finiteN\(recGet\(data, 'listen_port'\)\)/)
    expect(wg).toMatch(/finiteN\(recGet\(data, 'active_count'\)\)/)
    expect(wg).toMatch(/finiteN\(recGet\(data, 'peer_count'\)\)/)
    expect(wg).toMatch(/finiteN\(recGet\(data, 'keepalive_missing'\)\)/)
    ...
    expect(wg).toMatch(/finiteN\(recGet\(recGet\(readiness, 'peer_origin'\), 'foreign'/)
    expect(wg).toMatch(/finiteN\(recGet\(recGet\(readiness, 'peer_origin'\), 'total'/)
    expect(wg).toMatch(/finiteN\(recGet\(pingResult, 'reachable'/)
    expect(wg).toMatch(/finiteN\(recGet\(result, 'created'/)
    ...
    expect(wg).toMatch(/finiteText\(recGet\(data, 'public_key'/)
    ...
    expect(wg).toMatch(/finiteText\(recGet\(data, 'address'\), ''\) \|\| finiteText\(recGet\(data, 'subnet'\)/)
    ...
    expect(wg).toMatch(/finiteText\(recGet\(readiness, 'wan_interface'\), ''\) \|\| 'en0'/)
  ...
  it('Bookmarks leftover summary counts go through finite helpers', () => {
    ...
    expect(bookmarks).toMatch(/finiteText\(recGet\(bk, 'name'\), ''\) \|\| finiteText\(recGet\(bk, 'id'\), ''\)/)
  ...
  it('Backups leftover ports/files/mtime go through finite helpers', () => {
    ...
    expect(backups).toMatch(/finiteN\(recGet\(layer, 'pct'\)\)/)
  })
  ...
  it('App leftover service counts go through finiteN', () => {
    ...
    expect(app).toMatch(/finiteN\(recGet\(counts, 'ok'/)
    expect(app).toMatch(/finiteN\(recGet\(counts, 'warn'/)
    expect(app).toMatch(/finiteN\(recGet\(counts, 'down'/)
