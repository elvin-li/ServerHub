<template>
  <div>
    ...
    <div ref="smartPanel" v-if="smartModal" class="modal-bg" @click.self="smartModal=false" role="presentation">
      <div class="modal" style="max-width:880px" role="dialog" aria-modal="true" aria-labelledby="smart-modal-title">
        ...
        <div v-else>
          ...
          <div v-else class="table-wrap" style="max-height:400px;overflow:auto">
            <table class="dense">
              ...
              <tbody>
                <tr v-for="m in smartMerged" :key="m.id">
                  <td class="mono">
                    <strong>{{ m.id }}</strong>
...
import { getStorage, manageStorageDevice, setDiskPower, getSmartOverview, startSmartTest } from '../api/client'
...
const smartMerged = computed(() => {
  ...
  // Start from test devices (which have capabilities), enrich with storage SMART
  const merged = testDevices.map(td => {
    ...
    return {
      ...
      lastResult: td.last_result,
      logCount: td.log_count,
      failures: td.failures,
      progress: td.progress,
    }
  })
  // Also add storage disks that have no test entry (e.g. unsupported by smartctl)
...
async function runSmartTest(dev, kind) {
  ...
  try {
    ...
    if (j.ok) {
      // Refresh after a short delay to pick up the running test
      setTimeout(async () => {
