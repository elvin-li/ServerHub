<template>
  <div>
    ...
    <div class="toolbar" style="flex-wrap:wrap;gap:8px">
      ...
      <a v-if="data?.links?.immich" class="btn-link" :href="data.links.immich" target="_blank" rel="noopener">Immich</a>
      <a v-if="data?.links?.panel" class="btn-link" :href="data.links.panel" target="_blank" rel="noopener">{{ t('photoshub.status_panel') }}</a>
    </div>
    ...
    <div class="dash-grid" style="margin-bottom:14px" v-else-if="data">
        ...
        <table class="dense">
          <thead>
            <tr>
              ...
              <th>{{ t('photoshub.taken') }}</th>
              <th>{{ t('photoshub.type') }}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="a in pending?.assets || []" :key="a.id">
              <td><input type="checkbox" :value="a.id" v-model="selected" /></td>
              <td class="mono" style="font-size:12px">{{ a.originalFileName }}</td>
              <td class="meta">{{ a.localDateTime || '—' }}</td>
              <td>{{ a.type || '—' }}</td>
            </tr>
    ...
    <div class="card-block" v-if="lastAction">
      <div class="section-head">
        ...
        <span class="badge" :class="lastAction.ok ? 'ok' : 'down'">{{ lastAction.ok ? 'OK' : 'FAIL' }}</span>
      </div>
      <pre class="mono logbox">{{ lastAction.stdout || lastAction.stderr || '—' }}</pre>
    </div>
    ...
    <div class="card-block" style="margin-top:14px">
      <div class="section-head">
        ...
        <div class="tabs">
          <button v-for="n in logNames" :key="n" :class="{ active: logName===n }" @click="switchLog(n)">{{ n }}</button>
        </div>
      </div>
      <pre class="mono logbox">{{ (logData?.lines || []).join('\n') || '—' }}</pre>
    </div>
  </div>
</template>
