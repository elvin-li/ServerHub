...
def test_redis_like_port_is_not_probed_as_https():
    ...
    assert url is None, f"non-web port produced a URL: {url!r}"
...
def test_silent_port_is_not_probed_as_https():
    """A port that accepts TCP but never replies yields no URL, fast."""
    with _FakeServer(_silent) as srv:
        t0 = time.perf_counter()
        url = adaptive.guess_http_url(srv.port)
        elapsed = time.perf_counter() - t0
    assert url is None, f"silent port produced a URL: {url!r}"
    assert elapsed < BUDGET_S, (
...
def test_real_http_server_is_still_detected():
    ...
    try:
        ...
        httpd.shutdown()
        httpd.server_close()
    assert url is not None, "a live HTTP server was not detected as a web UI"
    assert url.startswith("http://"), url
    assert url.endswith(f":{port}"), url


def test_closed_port_yields_no_url():
    ...
    elapsed = time.perf_counter() - t0
    assert url is None
    assert elapsed < BUDGET_S, f"closed port took {elapsed * 1000:.1f}ms"


def test_known_non_http_ports_short_circuit():
    """The existing port blocklist still applies before any socket work."""
    for port in (5432, 3306, 1883):
        assert adaptive.guess_http_url(port) is None


def test_head_classifier_recognises_the_three_cases():
    """The wire-format classifier is what makes the decision falsifiable."""
    assert adaptive._classify_head(b"HTTP/1.1 200 OK") == "http"
    assert adaptive._classify_head(b"HTTP/1.0 401 Unauthorized") == "http"
    # TLS handshake record, then TLS alert record: both mean "speaks TLS".
    assert adaptive._classify_head(b"\x16\x03\x03\x00\x40\x02\x00\x00") == "tls"
    assert adaptive._classify_head(b"\x15\x03\x01\x00\x02\x02\x28") == "tls"
    # Redis, and anything else that is neither.
    assert adaptive._classify_head(b"-ERR unknown command") == ""
    assert adaptive._classify_head(b"") == ""
    assert adaptive._classify_head(b"SSH-2.0-OpenSSH_9.0") == ""
...
def test_https_port_answering_400_in_plaintext_is_detected_as_https(tmp_path):
    """The live 8281 regression: an HTTP/ status line can still mean TLS-only."""
    pem = _make_selfsigned(tmp_path)
    if pem is None:
        import pytest
    ...
    with _NginxLikeTLSServer(pem) as srv:
        url = adaptive.guess_http_url(srv.port)
    assert url is not None, "an HTTPS-only nginx port produced no URL"
    assert url.startswith("https://"), (
        f"got {url!r}; a plaintext 400 must not be trusted as a plaintext service"
    )
    assert url.endswith(f":{srv.port}"), url


def test_plain_400_without_tls_yields_no_url():
    """A 400 with no working TLS behind it is not a linkable web UI."""
    with _FakeServer(_plain_400) as srv:
        t0 = time.perf_counter()
        url = adaptive.guess_http_url(srv.port)
        elapsed = time.perf_counter() - t0
    assert url is None, f"a bare 400 produced a URL: {url!r}"
    assert elapsed < BUDGET_S, f"took {elapsed * 1000:.1f}ms"
