"""Thirteenth leftover sweep of VM sanitizer helpers.

vms12 sealed pool seams.  ``_isa`` / ``_as_text`` still stopped at
...
class Vms13LeftoverTests(unittest.TestCase):
    def test_isa_swallows_a_baseexception_class_bomb(self):
        self.assertFalse(vms_svc._isa(_ClassBaseBomb(), str))

    def test_as_text_does_not_leak_a_heap_address(self):
        self.assertEqual(vms_svc._as_text(object()), "")

    def test_as_text_recovers_str_storage_lying_bytes(self):
        self.assertEqual(vms_svc._as_text(_LyingBytesStr("ok")), "ok")

    def test_get_vms_does_not_500(self):
