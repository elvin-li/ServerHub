"""Thirteenth leftover sweep of WireGuard sanitizer helpers.

wg12 sealed write-path rows and apply-step answer shapes.  ``_isa`` /
``_as_text`` still stopped at ``except Exception`` and default object
``__repr__`` leaked heap addresses into JSON.
"""
...
class Wg13LeftoverTests(unittest.TestCase):
    def test_isa_swallows_a_baseexception_class_bomb(self):
        self.assertFalse(wireguard_svc._isa(_ClassBaseBomb(), str))

    def test_as_text_does_not_leak_a_heap_address(self):
        self.assertEqual(wireguard_svc._as_text(object()), "")

    def test_as_text_recovers_str_storage_lying_bytes(self):
        self.assertEqual(wireguard_svc._as_text(_LyingBytesStr("ok")), "ok")

    def test_get_wireguard_does_not_500(self):
        client = TestClient(app(), raise_server_exceptions=False)
        response = client.get("/api/wireguard")
