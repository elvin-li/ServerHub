...
class MacOSAdminTests(unittest.TestCase):
    def test_admin_command_uses_native_prompt_and_shell_quotes_arguments(self):
        ...
        self.assertIn("'/Users/example/Shared folder'", script)
        self.assertIn("'A; touch /tmp/no'", script)
        self.assertNotIn("password", script.lower())

    def test_admin_cancel_is_structured(self):
        with patch("hub.macos_admin.sh", return_value=(1, "", "execution error: User canceled. (-128)")):
            self.assertEqual(
