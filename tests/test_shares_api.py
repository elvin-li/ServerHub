...
class SharesAPIContractTests(unittest.TestCase):
    ...
    def test_local_token_can_read_but_cannot_mutate_sharing(self):
        overview = {"system_services": [], "smb": []}
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                patch("hub.auth.setup_required", return_value=False),
                patch("hub.auth.browser_authenticated", return_value=False),
                patch("hub.shares_svc.shares_overview", return_value=overview),
                patch("hub.shares_svc.set_system_service") as mutate,
            ):
        ...
        self.assertEqual((read_status, read_body), (200, overview))
