"""Thirteenth leftover sweep of Ollama sanitizer helpers.

ollama12 sealed listing seams and runner shapes.  ``_isa`` / ``_utf8_text``
...
class Ollama13LeftoverTests(unittest.TestCase):
    def test_isa_swallows_a_baseexception_class_bomb(self):
        self.assertFalse(ollama_svc._isa(_ClassBaseBomb(), str))

    def test_utf8_text_does_not_leak_a_heap_address(self):
        self.assertEqual(ollama_svc._utf8_text(object()), "")

    def test_utf8_text_recovers_str_storage_lying_bytes(self):
        self.assertEqual(ollama_svc._utf8_text(_LyingBytesStr("ok")), "ok")
    ...
    def test_control_flow_still_propagates_from_isa(self):
        ...
        with self.assertRaises(KeyboardInterrupt):
            ollama_svc._isa(_Ki(), dict)

    def test_jsonable_swallows_isoformat_getattr_baseexception(self):
        class _IsoBomb:
            @property
            def isoformat(self):
                raise LeftoverWatchdogTimeout("ollama isoformat watchdog")

        self.assertEqual(ollama_svc._jsonable(_IsoBomb()), "")
