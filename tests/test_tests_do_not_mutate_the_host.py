...
#: Route suffixes that reach the same installers over HTTP.  Driving them through
#: TestClient never mentions `install_native` by name, so the name-based scan below
#: cannot see it: a test that POSTs to /api/catalog/native-syncthing/install with
#: nothing patched runs `brew install` against this machine exactly as if it had
#: called the function.
INSTALL_ROUTE_MARKERS = ("/install", "/uninstall")


def _string_literals(tree: ast.Module) -> list[str]:
    """Every literal string, with f-strings flattened to their fixed parts.

    Route URLs in tests are usually f-strings (`f"/api/catalog/{app}/install"`),
    which are `JoinedStr` nodes rather than constants.  Reading only constants
    would miss exactly the form these tests are written in.
    """
    out: list[str] = []
...
def _install_routes(tree: ast.Module) -> set[str]:
    """String literals in the module that name an install/uninstall endpoint."""
    return {
        text
        for text in _string_literals(tree)
        if "/api/" in text and text.rstrip("/").endswith(INSTALL_ROUTE_MARKERS)
    }
...
class HostMutationTests(unittest.TestCase):
    ...
    def test_installer_tests_seal_the_execution_boundary(self):
        ...
        for path in self.modules:
            ...
            used = sorted((_referenced_names(tree) & DANGEROUS) | _install_routes(tree))
            if not used:
                continue
            if not (_patched_names(tree) & EXECUTOR_SEALS):
                offenders.append(
                    f"{path.relative_to(BASE)}: reaches {used} without "
