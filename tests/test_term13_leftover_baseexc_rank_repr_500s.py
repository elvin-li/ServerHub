"""Thirteenth leftover sweep of terminal sanitizer helpers.

term12 sealed home-seam, tailer and constant bombs.  ``_isa`` / ``_utf8_text``
...
class Term13LeftoverTests(unittest.TestCase):
    def test_isa_swallows_a_baseexception_class_bomb(self):
        self.assertFalse(terminal_svc._isa(_ClassBaseBomb(), str))

    def test_utf8_text_does_not_leak_a_heap_address(self):
        self.assertEqual(terminal_svc._utf8_text(object()), "")

    def test_utf8_text_recovers_str_storage_lying_bytes(self):
        self.assertEqual(terminal_svc._utf8_text(_LyingBytesStr("ok")), "ok")

    def test_get_terminal_does_not_500(self):
        client = TestClient(app(), raise_server_exceptions=False)
        response = client.get("/api/terminal")
