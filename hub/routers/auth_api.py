...
def _client(request: Request) -> str:
    # Loopback-aware (see auth.request_client): behind the local cloudflared /
    # nginx proxy this names the real visitor, so rate-limit buckets and audit
    # lines stop collapsing every proxied client into 127.0.0.1.  A direct
    # remote peer cannot influence it.
...
def auth_login(body: LoginBody, request: Request, response: Response):
    ...
    # Any configured account may sign in — admin and member alike.  The lookup
    # is exact-name; verify_account_password burns a dummy scrypt evaluation
...
def auth_change_password(body: ChangePasswordBody, request: Request, response: Response):
    """Change the signed-in account's password after re-authentication.
    ...
    Members rotate only their own credential (``username`` must be their own
    account).  An administrator may additionally rename themselves by sending
    a different ``username`` — the legacy behaviour of this endpoint.
    """
    if auth.setup_required():
        raise api_error("auth.setup_required")
    ...
    if not auth.is_admin(current_username) and username != current_username:
        # A member touches nobody's credential but their own.
    ...
    if not allowed:
        raise api_error("auth.rate_limited", retry=retry)
    # Always the *caller's own* current password, whatever name is being set:
    ...
    if not auth.verify_account_password(current_username, body.current_password):
        ...
        audit.record(
            ...
            username=current_username or username,
            client=client,
    ...
    try:
        if username == current_username:
            # In-place rotation for the caller's own account (admin or member).
            auth.set_account_password(username, body.new_password)
        else:
