...
CODES: dict[str, tuple[int, str]] = {
    ...
    # The inverse of worker_quarantined, and the loudest of the four states: the
    # note says the volume loses write barriers and the worker is writing to it
    # anyway.  "Running" is the dangerous answer here, not the healthy one.
    "immich.worker_running_while_quarantined": (
        409,
        "running while quarantined — it is writing to a volume that failed "
        "write-barrier testing; stop it before it writes more",
    ),
}
