...
_RESTORE_HINTS: tuple[tuple[str, str], ...] = (
    ...
    (
        # The panel's own pre-write rotations of services.yaml. Not written by this
        # module, but they are what recovered the admin account on this host after
        # the credential was lost from the live file, so the page should say how.
        "services.yaml.bak",
        "cp {path} " + str(CONFIG_FILE) + "  # then restart the panel",
    ),
    (
        "configs_",
